...
Before run Pbalign, a .fofn file for each raw data needs to generate. The .fofn file is a list of raw data name. To generate the fofn file, use ``` echo ${raw_data_directory}/0001/*/*.bas.h5 >> input_0001.fofn ```.

In the end, you will have 11 separated .fofn files, and each of them contains the name of .bas.h5 files. Please refer to the executable scripts on how we completed the task in a for loop. 

After generating the .fofn files, we can run Pbalignm, which maps PacBio reads to reference sequences. 
